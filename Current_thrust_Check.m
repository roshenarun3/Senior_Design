...
WeightFS = 1.2;
%Total Down Force
TotalDownForce = 9.81 * WeightFS * ( Payload + 8*D.Weight_g + B.Weight_kg*NB );
% Max thrust needed from motor estimation
Motor_MaxThrustEst = 2 / 8  * 9.81 * WeightFS * ( Payload + 8*D.Weight_g + B.Weight_kg*NB );

% For later - >  When body weight is calculated/estimated
% Motor_MaxThrustEst = 2 / 8  * 9.81 * WeightFS * ( Payload + 8*D.Weight_g + B.Weight_kg*NB + TOTALBODYWEIGHT )
% TotalDownForce = 9.81 * WeightFS * ( Payload + 8*D.Weight_g + B.Weight_kg*NB + TOTALBODYWEIGHT )
% Thrust check
